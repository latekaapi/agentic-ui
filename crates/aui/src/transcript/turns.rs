...
impl UserTurn {
    ...
    /// The bubble column's width cap as a fraction of the transcript width.
    ///
    /// Chat convention caps the bubble at 78% so a short message hugs the
    /// right edge with dead space beside it. A document-reading consumer
    /// pasting a circular into the pane wants the full column instead and
    /// passes `1.0`. Anything outside `0.0..=1.0` falls back to the 78%
    /// default rather than trusting the caller. The default keeps every
    /// existing caller byte-identical.
    pub fn max_width_fraction(mut self, fraction: f32) -> Self {
        self.max_width = if (0.0..=1.0).contains(&fraction) {
            fraction
        } else {
            USER_MAX
        };
...
impl RenderOnce for UserTurn {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        //
        // `ml_auto` is what puts the CAPPED COLUMN on the right. Without it
        // the column is left-anchored and only the bubble inside it is
        // right-aligned, so the whole turn sits in a band starting at the
        // left edge: a short message stops a cap's width short of the right
        // edge, and a message long enough to fill the band READS AS
        // LEFT-ALIGNED. Reported against Cockpit as "chat is misaligned",
        // and it is the same dead space reported earlier as ~300px beside
        // every sent message. Making the cap configurable (v0.1.1) changed
        // its width and never its anchor.
        let mut col = v_flex()
            .id(id.clone())
            .relative()
            .w_full()
            .max_w(relative(max_width))
            .ml_auto()
            .items_end()
            .gap(px(USER_GAP))
...
mod tests {
    use super::{
        ...
        user_turn, AssistantTurnAction, UserTurnAction, COPY_HOLD, USER_MAX,
    };
    ...
    #[test]
    fn user_width_defaults_to_the_chat_cap() {
        assert_eq!(user_turn("t", "hi").max_width, USER_MAX);
        assert_eq!(USER_MAX, 0.78);
    }

    #[test]
    fn user_width_override_is_applied() {
        assert_eq!(user_turn("t", "hi").max_width_fraction(1.0).max_width, 1.0);
        assert_eq!(user_turn("t", "hi").max_width_fraction(0.5).max_width, 0.5);
    }
