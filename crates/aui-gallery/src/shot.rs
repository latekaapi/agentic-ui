//! `--screenshot`: render the window off-screen after a couple of frames,
//! downsample to logical pixels (the reference PNGs are 1×) and quit.
...
pub fn capture_and_quit(handle: WindowHandle<Root>, path: PathBuf, delay: Duration, cx: &mut App) {
    cx.spawn(async move |cx| {
        ...
        match result {
            Ok(Ok((w, h))) => println!("wrote {} ({w}×{h})", path.display()),
