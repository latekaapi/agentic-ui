...
//! block, a table, a quote, a rule and an image placeholder.

use aui::transcript::{markdown, ProseStyle};
...
pub fn build(_window: &mut Window, cx: &mut App) -> AnyElement {
    ...
    v_flex()
        .w_full()
        .gap(px(BLOCK_GAP))
        .child(markdown("card-markdown", SAMPLE, style))
        .child(
            div()
                .max_w(px(NOTE_MEASURE))
                .ui(scale::FS_12)
                .text_color(p.ink_3)
                .child("Headings step down the token ramp; links underline in accent and click through on_link; fences reuse the code block; tables scroll inside their own frame; images are placeholder tiles and never fetch."),
        )
        .into_any_element()
}
