...
### 3.2 Turns (card 31)
- User turn: right-aligned, max 72% width, surface-3, radius 12/12/4/12, padding 9 13, 13 px/1.5. Attachments strip above (30 px chips with 18 px icon tile). Mention chips inside text: surface-3 ground, accent-ink mono 12. A reported timestamp draws a mono 11 ink-4 how-long-ago caption under the bubble (`5m ago`, via `format_age`). Hover reveals edit / copy / re-send xs ghost buttons to the left.
- Assistant turn: full pane width, no bubble, 13.5 px/1.65 ink. Streaming: chunks fade + 3 px rise (StreamReveal); caret 2 × 15 px accent blinking 1 s steps(2). Hover toolbar (copy, retry, fork, bookmark) appears above-right at opacity 1 with 4 px rise, never shifting layout. Footer meta mono 11 ink-4: model · duration · tokens · cost · age (a reported timestamp appends `2m ago` via `format_age`; no timestamp draws no cell).
