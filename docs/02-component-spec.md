...
- Host actions: `ToolCard::actions` takes `ToolCardAction { id, label, icon? }` descriptions, drawn as xs ghost buttons after the duration in the order given; a press emits `ToolCardIntent::Action(index)`. What the action does is the host's call. With no actions the header is unchanged.
- Grouped cards: `ToolGroup::card_actions(index, actions)` (or one `card_action(index, action)`) puts the same trailing header actions on one call's inner card; a press emits `ToolGroupIntent::Call { index, intent: ToolCardIntent::Action(action) }`, naming both the card and the action. Calls with no actions render exactly as before.
...
- Host action: `CodeBlock::host_action(index, button)` appends the host's labelled control after Copy; pressing it emits `CodeBlockAction::HostAction { index, language, code }`, carrying everything the host needs to act on the block. Without it the header is unchanged. `runnable_command(lang, code)` reports the command a fence carries: `sh` / `bash` / `zsh` / `shell` / `terminal` fences come back whole; `console` and untagged all-`$`-prompt fences contribute their prompt lines with the prefixes stripped and the recorded output dropped; anything else yields `None`.
- Markdown fences: `Markdown::fence_action(index, button)` puts the host's labelled control on fence `index` after Copy; `markdown_fences(source)` lists each fence as `MarkdownFence { index, language, code }` with indices numbering every fenced block in document order from 0 (tables, lists and inline code take no number). Pressing a fence button emits `CodeBlockAction::HostAction { index, language, code }` through `Markdown::on_code_action`, so the host can tell which fence was pressed. Which fences get a button is the host's call. `AssistantTurn` / `UserTurn` forward both as `fence_action` / `on_code_action`. Fences with no button render exactly as before.
