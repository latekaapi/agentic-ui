...
- The segmented control (card 52) slides one surface-1 thumb between segments, the way the tab strip slides its indicator: each segment reports its bounds through `ElementExt::on_prepaint` into keyed state, and the thumb is an absolutely positioned first child sprung to last frame's bounds on the layout spring. Text-measured widths still are not known at build time, so the very first frame draws no thumb and requests another; reduced motion snaps because `spring_px` resolves instantly.
- Code blocks are coloured by the small lexer in `transcript::syntax`. The optional `aui/tree-sitter` feature routes `syntax_runs_in` through gpui-kit's `SyntaxHighlighter` for rust / typescript / tsx / json / python / bash (`cargo build -p aui-gallery --features aui/tree-sitter`); it is off by default and nothing in the gallery enables it. Two limits: the highlighter is built per line, so a construct spanning lines (an open block comment, a multi-line string) is classified per line exactly as the lexer does it; and capture names are read back through a sentinel `HighlightStyle` because gpui-component's `styles()` is the only public entry point.
